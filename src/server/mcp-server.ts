...
export async function createAtelMcpServer(args: {
  ...
  server.registerTool('atel_agent_search', { description: 'Search registry for ATEL agents and capabilities.', inputSchema: AgentSearchInputSchema.shape }, async (input) => asToolResult(await invoke('atel_agent_search', input)));
  ...
  server.registerTool('atel_contacts_list', { description: 'List server-canonical ATEL contacts.' }, async () => asToolResult(await invoke('atel_contacts_list')));
  server.registerTool('atel_inbox_list', { description: 'List ATEL inbox/relay messages.' }, async () => asToolResult(await invoke('atel_inbox_list')));
  server.registerTool('atel_send_message', { description: 'Send a text P2P message through ATEL relay.', inputSchema: SendMessageInputSchema.shape }, async (input) => asToolResult(await invoke('atel_send_message', input)));
  server.registerTool('atel_ack', { description: 'Acknowledge inbox messages.', inputSchema: AckInputSchema.shape }, async (input) => asToolResult(await invoke('atel_ack', input)));
  ...
  server.registerTool('atel_order_timeline', { description: 'Get the timeline/domain events for an order.', inputSchema: orderIdInput() }, async (input) => asToolResult(await invoke('atel_order_timeline', input)));
  ...
  server.registerTool('atel_audit_order_get', { description: 'Read MCP audit events for an order.', inputSchema: AuditOrderQueryInputSchema.shape }, async (input) => asToolResult(await invoke('atel_audit_order_get', input)));
  server.registerTool('atel_audit_session_get', { description: 'Read MCP audit events for the current or specified session.', inputSchema: AuditSessionQueryInputSchema.shape }, async (input) => asToolResult(await invoke('atel_audit_session_get', input)));
