...
export async function createAtelMcpServer(args: {
  ...
  server.registerTool('atel_recover', { description: 'Look up DID by recoveryCode (returned at atel_register_user time). UNAUTHENTICATED. Use case: you kept secretKey but forgot which DID it belongs to.', inputSchema: RecoverInputSchema.shape }, async (input) => tryInvoke(invoke, 'atel_recover', input));
  server.registerTool('atel_secret_key_recover', { description: 'Recover the full secretKey for a DID using its recoveryCode. UNAUTHENTICATED. Returns {did, secretKey (base64), hint}. Requires the DID was registered AFTER server-side KEK backup was enabled (older DIDs return 410 Gone). Treat the recoveryCode as equally sensitive as secretKey going forward.', inputSchema: RecoverInputSchema.shape }, async (input) => tryInvoke(invoke, 'atel_secret_key_recover', input));
  ...
  server.registerTool('atel_order_create', { description: 'Create an ATEL order (escrow + milestone flow). The `chain` field selects settlement chain: omit for default base (~4-8min), pass "fast-coop" when the user requests fast / fast-coop / 走 fast / 用 fast 链下单 (~1min gasless settle on Fast Network), pass "bsc" for BSC. See chain field description for full mapping. Do not confuse chain="fast-coop" here (order escrow) with atel_fast_transfer (direct P2P USDC transfer).', inputSchema: OrderCreateInputSchema.shape }, async (input) => tryInvoke(invoke, 'atel_order_create', input));
  ...
  server.registerTool('atel_a2b_search', { description: 'Search Bitrefill gift cards. Server enforces limit=30 (the SKILL.md drift case). Filter by country (e.g. ZA, US). Call atel_a2b_countries first to discover valid country codes.', inputSchema: A2bSearchInputSchema.shape }, async (input) => tryInvoke(invoke, 'atel_a2b_search', input));
  server.registerTool('atel_a2b_countries', { description: 'List the country codes Bitrefill supports for product search. Returns {countries:[{code, name}]}. Cheap (24h server cache); run before guessing country codes for atel_a2b_search.' }, async () => tryInvoke(invoke, 'atel_a2b_countries'));
