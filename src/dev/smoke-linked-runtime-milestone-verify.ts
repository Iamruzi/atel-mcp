...
async function main() {
  ...
  const requester = createClient('atel-mcp-requester-linked-verify', serverUrl, requesterToken);
  const requesterLinked = createClient('atel-mcp-requester-linked-verify-hop', serverUrl, requesterToken, {
    'x-atel-runtime-backend': 'linked-runtime',
    'x-atel-user-mode': 'mcp-plus-runtime',
  });
  const executor = createClient('atel-mcp-executor-linked-verify', serverUrl, executorToken);
  ...
  try {
    ...
    const bind = parseToolText(await requester.client.callTool({
      ...
      arguments: {
        runtimeDid: 'did:atel:ed25519:self-runtime-verify-smoke',
    ...
    const create = parseToolText(await requester.client.callTool({
      ...
      arguments: {
        executorDid,
        capabilityType,
        description: `linked-runtime-milestone-verify-smoke-${Date.now()}`,
        priceUsdc: 0,
      },
    }));
    ...
    const submit = parseToolText(await executor.client.callTool({
      ...
      arguments: {
        orderId,
        index: 0,
        content: 'linked-runtime milestone verify smoke content',
      },
    }));

    const verify = parseToolText(await requesterLinked.client.callTool({
    ...
    const ok = milestones?.currentMilestone === 1 && milestones?.milestones?.[0]?.status === 'verified';
    ...
    console.log(JSON.stringify({ ok, bind, create, accept, submit, verify, milestones, unbind }, null, 2));
