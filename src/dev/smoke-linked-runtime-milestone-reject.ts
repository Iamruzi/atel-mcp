...
async function main() {
  ...
  const requester = createClient('atel-mcp-requester-linked-reject', serverUrl, requesterToken);
  const requesterLinked = createClient('atel-mcp-requester-linked-reject-hop', serverUrl, requesterToken, {
    'x-atel-runtime-backend': 'linked-runtime',
    'x-atel-user-mode': 'mcp-plus-runtime',
  });
  const executor = createClient('atel-mcp-executor-linked-reject', serverUrl, executorToken);
  ...
  try {
    ...
    const bind = parseToolText(await requester.client.callTool({
      ...
      arguments: {
        runtimeDid: 'did:atel:ed25519:self-runtime-reject-smoke',
    ...
    const create = parseToolText(await requester.client.callTool({
      ...
      arguments: {
        executorDid,
        capabilityType,
        description: `linked-runtime-milestone-reject-smoke-${Date.now()}`,
        priceUsdc: 0,
      },
    }));
    ...
    const accept = parseToolText(await executor.client.callTool({
      name: 'atel_order_accept',
      arguments: { orderId },
    }));

    const submit = parseToolText(await executor.client.callTool({
      name: 'atel_milestone_submit',
      arguments: {
        orderId,
        index: 0,
        content: 'linked-runtime milestone reject smoke content',
      },
    }));

    const reject = parseToolText(await requesterLinked.client.callTool({
      name: 'atel_milestone_reject',
      arguments: {
        orderId,
        index: 0,
        content: 'needs tighter evidence',
      },
    }));

    const milestones = parseToolText(await requester.client.callTool({
      name: 'atel_milestone_list',
      arguments: { orderId },
    }));

    const ok = milestones?.currentMilestone === 0 && milestones?.milestones?.[0]?.status === 'rejected';
    ...
    console.log(JSON.stringify({ ok, bind, create, accept, submit, reject, milestones, unbind }, null, 2));
