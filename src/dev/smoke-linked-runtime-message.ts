...
async function main() {
  ...
  const requesterLinked = createClient('atel-mcp-requester-linked', serverUrl, requesterToken, {
    'x-atel-runtime-backend': 'linked-runtime',
    'x-atel-user-mode': 'mcp-plus-runtime',
  });
  const executor = createClient('atel-mcp-executor', serverUrl, executorToken);
  const smokeText = `linked-runtime-smoke-${Date.now()}`;
  ...
  try {
    ...
    const send = parseToolText(await requesterLinked.client.callTool({
    ...
    const inbox = parseToolText(await executor.client.callTool({
      name: 'atel_inbox_list',
      arguments: {},
    }));

    const inboxContainsSmokeText = JSON.stringify(inbox).includes(smokeText);
    ...
    console.log(JSON.stringify({
      ok: inboxContainsSmokeText,
      smokeText,
      bind,
      send,
      inboxContainsSmokeText,
      unbind,
    }, null, 2));

    if (!inboxContainsSmokeText) {
      process.exitCode = 1;
    }
  } finally {
