...
async function main() {
  ...
  const requester = createClient('atel-mcp-requester-ms', serverUrl, requesterToken);
  const executor = createClient('atel-mcp-executor-ms', serverUrl, executorToken);
  const executorLinked = createClient('atel-mcp-executor-linked-submit', serverUrl, executorToken, {
  ...
  try {
    ...
    const create = parseToolText(await requester.client.callTool({
      ...
      arguments: {
        executorDid,
        capabilityType,
        description: `linked-runtime-milestone-submit-smoke-${Date.now()}`,
        priceUsdc: 0,
      },
    }));
    ...
    const submit = parseToolText(await executorLinked.client.callTool({
      name: 'atel_milestone_submit',
      arguments: {
        orderId,
        index: 0,
        content: 'linked-runtime milestone submit smoke content',
      },
    }));
    ...
    const submitObserved = milestoneText.includes('linked-runtime milestone submit smoke content');
    ...
    console.log(JSON.stringify({ ok: submitObserved, bind, create, accept, submit, milestones, unbind }, null, 2));
    if (!submitObserved) process.exitCode = 1;
  } finally {
